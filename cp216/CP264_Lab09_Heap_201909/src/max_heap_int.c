...
// it is in its correct location in the heap.
{
	...
	int indexval = source->values[index];
	int parentval = source->values[parent];

	while ((index > 0) && (parentval < indexval)) {
		heap_swap(&source->values[index], &source->values[parent]);

		index = parent;
		parent = (index - 1) / 2;

		indexval = source->values[index];
		parentval = source->values[parent];
	}
#ifdef DEBUG_LEVEL_01
