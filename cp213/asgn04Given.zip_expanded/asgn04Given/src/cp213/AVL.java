...
public class AVL<T extends Comparable<T>> extends BST<T> {
    ...
    private TreeNode<T> rotateLeft(final TreeNode<T> node) {
    	final TreeNode<T> num = node.getRight();
    ...
    private TreeNode<T> rotateRight(final TreeNode<T> node) {
    	final TreeNode<T> num = node.getLeft();
    ...
    protected TreeNode<T> insertAux(TreeNode<T> node, final CountedItem<T> data) {
		...
		if (node == null) {
			...
			if (result > 0) {
				...
				toBalance = this.balance(node);

				if (toBalance > 1 && this.balance(node.getLeft()) >= 0) {
					
					node = this.rotateRight(node);
				} else if (toBalance > 1 && this.balance(node.getLeft()) < 0) {
				...
				if (toBalance < -1 && this.balance(node.getRight()) <= 0) {
					node = this.rotateLeft(node);
				} else if (toBalance < -1 && this.balance(node.getRight()) > 0) {
